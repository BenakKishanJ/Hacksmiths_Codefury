...
export class ArtworkModel {
  ...
  async createArtwork(artworkData: Omit<Artwork, '_id' | 'createdAt' | 'updatedAt'>): Promise<Artwork> {
    const now = new Date();
  ...
  async getArtworksByArtist(artistId: string | ObjectId, limit = 20, skip = 0): Promise<Artwork[]> {
  ...
  async getArtworksByArtform(artformId: string | ObjectId, limit = 20, skip = 0): Promise<Artwork[]> {
  ...
  // Update artwork
  async updateArtwork(id: string | ObjectId, updateData: Partial<Artwork>): Promise<boolean> {
    const _id = typeof id === 'string' ? new ObjectId(id) : id;
  ...
  async markAsInAuction(id: string | ObjectId, auctionId: string | ObjectId): Promise<boolean> {
    const _id = typeof id === 'string' ? new ObjectId(id) : id;
